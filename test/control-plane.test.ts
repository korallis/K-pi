...
import { verifyChain } from "../extensions/append-log.ts";
import {
  createStatusWidget,
...
async function createRun(directory: string): Promise<string> {
  const runDirectory = join(directory, ".pi", "runs", "2026-08-31-status");
...
test("kpi stop appends a terminal BLOCKED event", async () => {
  await withFixture(async (directory) => {
    ...
    const lines = (await readFile(join(runDirectory, "events.jsonl"), "utf8"))
      .trimEnd()
      .split("\n");
