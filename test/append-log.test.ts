...
const eventSchema = JSON.parse(
  await readFile(new URL("../schemas/event.schema.json", import.meta.url), "utf8"),
) as JsonSchema;
...
test("secrets are redacted inside allowed semantic fields", async () => {
  await withEventLog(async (path) => {
    ...
    for (const canary of canaries) {
      ...
      assert.doesNotMatch(JSON.stringify(record), new RegExp(canary.replaceAll(/[.*+?^${}()|[\]\\]/gu, "\\$&"), "u"));
    }
  ...
  await withEventLog(async (path) => {
    ...
    for (const payload of [
      ...
      await assert.rejects(
        ...
        (error: unknown) =>
          error instanceof ForbiddenEventPayloadError &&
          /Forbidden event payload key/u.test(error.message),
      );
    }
