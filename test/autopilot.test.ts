...
import type { GraphAgentSessionFactory } from "../extensions/graph/engine.ts";
        ...
        async prompt(prompt) {
          ...
          if (currentNode === "implement") {
            ...
            lastAssistantText = JSON.stringify({
              ...
              ac_results: ["AC-01", "AC-02", "AC-03", "AC-04", "AC-05"].map(
                (id) => ({ id, passed: true }),
              ),
            });
...
test("narrative acceptance criteria refuse forced autopilot before graph load", async () => {
  ...
  try {
    ...
    const events = await readFile(
      join(directory, ".pi", "runs", jobId, "events.jsonl"),
      "utf8",
    );
    assert.match(events, /"type":"ac\.refused"/u);
    await assert.rejects(
      readdir(join(directory, ".pi", "runs", jobId, "graph")),
      { code: "ENOENT" },
    );
  } finally {
    await rm(directory, { recursive: true, force: true });
...
test("autopilot healthcheck reaches DONE with one commit and no human node", async () => {
  ...
  try {
    ...
    const taskDocument = JSON.parse(
      await readFile(
        join(directory, ".pi", "runs", jobId, "task.json"),
        "utf8",
      ),
    ) as { acceptance: Array<{ bounds?: unknown; check?: unknown }> };
    assert.equal(taskDocument.acceptance.length, 5);
    assert.ok(
      taskDocument.acceptance.every(
        (criterion) =>
          criterion.check !== undefined && criterion.bounds !== undefined,
      ),
    );
    assert.equal(
      await git(directory, "rev-parse", "--abbrev-ref", "HEAD"),
      `kpi/${jobId}`,
    );
    assert.equal(
...
test("an autopilot write outside bounds stops UNSAFE without a commit", async () => {
  ...
  const harness = commandHarness(
    directory,
    autoSessions(directory, executed, { violateBounds: true }),
    jobId,
  );
  try {
    const task = await readFile(join(directory, "task.txt"), "utf8");
