...
import { EVENT_TYPES } from "../extensions/append-log.ts";
import {
  validateJsonSchema,
  type JsonSchema,
  ...
  switch (type) {
    ...
    case "agent.spawned":
      return {
        ...
        role: "reviewer",
        pid: 123,
        session_path: ".pi/runs/job/agents/reviewer-1.jsonl",
        status: "running",
      };
    case "agent.message":
      return {
...
test("event schema rejects cross-type and research vocabulary drift", async () => {
  ...
  assertInvalid(
    { ...eventPayload("research.started"), network_state: "degraded" },
    schema,
  );
  assertInvalid({ ...eventPayload("research.started"), mode: "native" }, schema);
  assertInvalid(
    { ...eventPayload("agent.message"), headers: { authorization: "secret" } },
    schema,
  );
});
