...
import { AccountBalancer, DEFAULT_FALLBACK_CHAIN } from "../extensions/accounts/balancer.ts";
import { classifyProviderFailure, DEFAULT_COOLDOWN_MS } from "../extensions/accounts/errors.ts";
import type { AccountsDocument } from "../extensions/accounts/store.ts";
import { renderAccountsWidget } from "../extensions/accounts/widget.ts";
import { BackgroundBus, type WorkerLauncher } from "../extensions/bus/spawn.ts";
import { appendEvent } from "../extensions/append-log.ts";
import { refreshCursorModels, registerCursorProvider } from "../extensions/cursor/provider.ts";
import { KnowledgeGraphStore } from "../extensions/kg/store.ts";
import { assertMinimalistBounds } from "../extensions/minimalist.ts";
import { registerPrintProfile } from "../extensions/print-profile.ts";
import { formatEventEntry } from "../extensions/renderers.ts";
import { conductResearch } from "../extensions/research/gate.ts";
import { exaSearch } from "../extensions/research/exa.ts";
import { assertClaimInModule, assertDuneStack, scaffoldModule, type DuneStack } from "../extensions/stack.ts";
import type { Task } from "../extensions/run-store.ts";
import { createKModePlan, assertShipApproved } from "../kstack/mode.ts";
import { assertKnownModels, createSuggestedModels } from "../kstack/models.ts";
...
test("429 usage limit classifies to the default cooldown", () => {
  ...
  assert.equal(classifyProviderFailure({ status: 403, headers: { "x-error": "quota exhausted" } }, now)?.kind, "cooldown");
});
...
test("Cursor registers its id and refreshes a mocked live array", async () => {
  ...
  registerCursorProvider({ registerProvider(providerId: string, providerConfig: typeof config) { id = providerId; config = providerConfig; } } as unknown as ExtensionAPI);
...
test("knowledge graph proposals serialize and acceptance bumps rev", async () => {
  ...
  try {
    ...
    const node = { id: "claim", kind: "decision", source_ids: ["src"], status: "proposed" as const, observed_at: new Date(0).toISOString() };
...
test("background bus caps workers, writers, messages, and leases", async () => {
  ...
  let pid = 1;
  const launcher: WorkerLauncher = async () => ({ pid: pid++, send: async (message) => { messages.push(message); }, stop: async () => undefined });
  try {
    const bus = new BackgroundBus(directory, join(directory, ".pi", "runs", "job"), launcher);
...
test("research caps results and falls back after a preferred 429", async () => {
  ...
  const fetchMock: typeof fetch = async (input) => {
    ...
    return new Response(JSON.stringify({ results: [{ title: "A", url: "https://a", snippet: "one" }, { title: "B", url: "https://b", snippet: "two" }] }), { status: 200 });
  };
  const capped = await exaSearch("q", "key", { numResults: 99, fetch: async (_input, init) => {
  ...
  try {
    ...
    const document = await conductResearch(directory, directory, task, { exaKey: "x", perplexityKey: "p", fetch: fetchMock });
...
test("Dune stack rejects generic maps and outside-module claims", () => {
  const stack: DuneStack = {
    ...
    root: "src",
    scaffold_first: true,
    modules: [{ id: "auth", purpose: "login and sessions", folder: "src/auth", interface: "src/auth/api.ts", allowed_paths: ["src/auth/**", "test/auth/**"], depends_on: [] }],
  };
  ...
  stack.modules[0] = { ...stack.modules[0], id: "helpers", folder: "src/helpers", interface: "src/helpers/index.ts", allowed_paths: ["src/helpers/**", "test/helpers/**"], purpose: "misc" };
...
test("print mode removes mutation tools", () => {
  ...
  registerPrintProfile({ on(_event: string, handler: typeof start) { start = handler; }, getActiveTools() { return active; }, setActiveTools(tools: string[]) { active = tools; } } as unknown as ExtensionAPI);
...
test("forbidden runtime dependencies and official model overlays remain absent", async () => {
  const packageDocument = JSON.parse(await readFile("package.json", "utf8")) as { dependencies?: Record<string, string> };
  const forbidden = ["pstack", "open-pstack", "pi-pstack", "pi-intercom", "pi-mesh", "pi-bus", "exa-js", "@perplexity-ai/perplexity_ai"];
  for (const name of forbidden) assert.equal(packageDocument.dependencies?.[name], undefined);
  const source = await readFile("extensions/index.ts", "utf8");
