...
test("implementer tools cannot write the reviewer verdict", () => {
	const decision = evaluateToolCall(write("/fixture/.kpi/runs/job-1/verdict.json"), {
		cwd,
		policy,
		writeAllow: [".kpi/runs/**"],
	});
...
test("implementer tools cannot write deterministic release approval", () => {
	const decision = evaluateToolCall(write("/fixture/.kpi/runs/job-1/release.approved"), {
		cwd,
		policy,
		writeAllow: [".kpi/runs/**"],
	});
	assert.equal(decision.allowed, false);
	assert.match(decision.reason ?? "", /reserved release\.approved for the release\.set node/u);
});

test("only the reserved run artifact names are protected", () => {
	const runWriteAllow = [".kpi/runs/**", ".pi/runs/**"];
	for (const path of [
		...
		assert.deepEqual(
			evaluateToolCall(write(path), { cwd, policy, writeAllow: runWriteAllow }),
			{ allowed: true },
			path,
		);
	}
