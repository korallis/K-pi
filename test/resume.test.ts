...
import { resumeLoop } from "../extensions/gated-loop.ts";
import { GraphEngine, type GraphAgentSessionFactory } from "../extensions/graph/engine.ts";
...
function factory(executed: string[]): GraphAgentSessionFactory {
  ...
  return async () => ({
    session: {
      ...
      async prompt(prompt: string) { executed.push(prompt.split("\n", 1)[0]); },
...
test("restored checkpoint does not rerun completed plan and implement nodes", async () => {
  ...
  const graph: GraphDefinition = {
    ...
    id: "resume-check",
    entry: "plan",
    nodes: ["plan", "implement", "test"].map((id) => ({ id, type: "agent" as const, prompt: id, context: { mode: "isolated" as const }, tools: ["read"], readOnly: true })),
    edges: [{ from: "plan", to: "implement" }, { from: "implement", to: "test" }, { from: "test", to: "__end__" }],
  ...
  try {
    const before: string[] = [];
    const engine = new GraphEngine(graph, { projectRoot: directory, jobId: "resume-job", createAgentSession: factory(before) });
    ...
    const after: string[] = [];
    const restored = await GraphEngine.restore(graph, { projectRoot: directory, jobId: "resume-job", createAgentSession: factory(after) });
...
test("resuming an already-DONE job is idempotent", async () => {
  ...
  const run = join(directory, ".pi", "runs", jobId);
  const task: Task = {
    job_id: jobId,
    mode: "gated",
    goal: "done",
