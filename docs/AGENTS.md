...
This directory is the source of truth for **K-π**, a standalone coding-agent harness maintained as a fork of Pi. The whole harness lives in this repository under `packages/`; K-π's own runtime lives in `packages/coding-agent/src/kpi/`. K-π is not a Pi package, is not installed into Pi, and has no Pi peer dependency. Fork policy: [`UPSTREAM.md`](../UPSTREAM.md).

Read in this order before writing code:

1. [`START-HERE.md`](../START-HERE.md) then [`docs/BUILD-PROMPT.md`](BUILD-PROMPT.md)
2. [`UPSTREAM.md`](../UPSTREAM.md) — fork base, sync policy, patched-file register
3. [`docs/PRD.md`](PRD.md) — stories US-01–US-30
4. [`docs/spec.md`](spec.md) — architecture, file contracts, schemas, APIs
5. [`docs/visual-targets.md`](visual-targets.md) + `docs/visual/*.jpg`
6. [`docs/kstack.md`](kstack.md) + [`docs/model-ladder.md`](model-ladder.md)
7. [`docs/research.md`](research.md) + [`docs/dune-architecture.md`](dune-architecture.md)
8. [`docs/minimalist.md`](minimalist.md) + [`docs/agents-bus.md`](agents-bus.md)
9. [`docs/roadmap.md`](roadmap.md) then [`docs/implementation-plan.md`](implementation-plan.md) — historical baseline only; their checkboxes are a build record, not completion evidence
10. [`docs/remediation-research.md`](remediation-research.md)
11. [`docs/remediation-plan.md`](remediation-plan.md) — active queue, RP-01A first
...
- Upstream changes come only through the `upstream` remote, reviewed per [`UPSTREAM.md`](../UPSTREAM.md). Prefer `packages/coding-agent/src/kpi/**` over patching upstream files; every upstream file you must patch goes in the [`UPSTREAM.md`](../UPSTREAM.md) patched-file register.
...
- K-stack is a vendored overlay on Cursor pstack. Commands: `/setup-kstack`, `/k-mode`. No Cursor Cloud agents. Models only from the K-π pool. Edit `kstack/overlay/`, never `kstack/upstream/` or by-hand `kstack/generated/`. Refresh with `npm run kstack:sync`. See [`docs/kstack.md`](kstack.md) §2.
...
- No Cursor-style subagents. Workers are background K-π sessions. They talk only via `communicate` (`sendUserMessage` / RPC prompt). One writer at a time. `claim_path` before edits. See [`docs/agents-bus.md`](agents-bus.md).
...
- Moving to a newer upstream release is a reviewed merge per [`UPSTREAM.md`](../UPSTREAM.md), never an automated bump.
