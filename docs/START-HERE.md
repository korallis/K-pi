...
Drop the whole `k-pi/` tree into an empty repo. Keep `docs/` as `docs/` or copy it to `specs/000-k-pi/`. Then give an agent [`docs/BUILD-PROMPT.md`](BUILD-PROMPT.md). In this repository `docs/` stays at `docs/`, so every pointer below resolves from the repository root and the active queue is always [`docs/remediation-plan.md`](remediation-plan.md).
...
5. `/setup-kstack` suggests a role map from [`docs/model-ladder.md`](model-ladder.md) (apply or tweak), then offers Exa and Perplexity keys (save either, both, or skip).
...
1. This file
2. [`docs/AGENTS.md`](AGENTS.md)
3. [`docs/PRD.md`](PRD.md) — stories US-01–US-30
4. [`docs/spec.md`](spec.md)
...
9. [`docs/roadmap.md`](roadmap.md) + [`docs/implementation-plan.md`](implementation-plan.md) — historical baseline only; their checkboxes are a build record, not completion evidence
10. [`docs/remediation-research.md`](remediation-research.md)
11. [`docs/remediation-plan.md`](remediation-plan.md) — active queue, RP-00 first
...
Do **one work package at a time**: the lowest incomplete dependency-ready `RP-##` in [`docs/remediation-plan.md`](remediation-plan.md), starting at RP-00. Do not skip to K-stack or external research before the graph exists.

Visual source for the boards: https://x.com/av1dlive/status/2092622516544270781

Paste-ready agent prompt: [`docs/BUILD-PROMPT.md`](BUILD-PROMPT.md).
