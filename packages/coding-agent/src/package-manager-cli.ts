...
// K-π fork guard. Every self-update path below resolves the upstream Pi release
// from pi.dev and installs it over the running install, which would replace this
// fork with upstream Pi. The fork is built from its own checkout and is never
// published to a registry, so `update --self`, `update --all`, `update self`,
// `update pi` and the bare `update` default are rejected locally before any
// network or package-manager work happens. Extension and model updates are
// unaffected. Restore the upstream behaviour by deleting this block and its two
// call sites once APP_NAME is "pi" again.
const IS_UPSTREAM_PI = APP_NAME === "pi";

function printForkSelfUpdateRejected(): void {
	console.error(chalk.red(`error: ${APP_NAME} does not self-update.`));
	console.error(
		chalk.dim(
			`${APP_TITLE} is a maintained fork of Pi, built from its own checkout and never published as a release.`,
		),
	);
	console.error(chalk.dim(`Update the harness from that checkout: git pull && npm ci && npm run build`));
	console.error(chalk.dim(`Updatable from here: ${APP_NAME} update --extensions, ${APP_NAME} update --models`));
}
...
${APP_TITLE} is a maintained fork of Pi built from its own checkout, so it never
updates itself from a registry. Update the harness from that checkout instead.
...
export async function handlePackageCommand(
	...
	if (!IS_UPSTREAM_PI && options.command === "update") {
		const target = options.updateTarget ?? { type: "self" };
		if (updateTargetIncludesSelf(target)) {
			printForkSelfUpdateRejected();
			process.exitCode = 1;
			return true;
		}
	}
