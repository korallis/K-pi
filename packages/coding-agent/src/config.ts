...
/**
 * Get path to the bundled K-π resource root (shipped with package).
 *
 * Holds the first-party K-π loop's non-package resources — extensions, graphs,
 * prompts, schemas, skills, templates, themes, kstack — in one sibling layout so
 * source and dist resolve identically:
 * - For Bun binary: kpi/ next to executable
 * - For Node.js (dist/): dist/kpi/
 * - For tsx (src/): src/kpi/
 */
export function getKpiResourceDir(): string {
	if (isBunBinary) {
		return join(getPackageDir(), "kpi");
	}
	const packageDir = getPackageDir();
	const srcOrDist = existsSync(join(packageDir, "src")) ? "src" : "dist";
	return join(packageDir, srcOrDist, "kpi");
}
...
interface PackageJson {
	...
	piConfig?: {
		name?: string;
		/** Human-facing product name. Falls back to `name` when unset. */
		title?: string;
		configDir?: string;
	};
}
