...
function validate(value: unknown, schema: JsonSchema, path: string, root: JsonSchema): string[] {
	...
	if (resolved.enum !== undefined && !resolved.enum.some((candidate) => isDeepStrictEqual(candidate, value))) {
	...
	if (resolved.oneOf !== undefined) {
		const branches = resolved.oneOf.map((branch) => validate(value, branch, path, root));
		...
		if (matched === 0) {
			...
			errors.push(`${path} must match one of ${branches.length} variants`, ...closest);
		} else if (matched > 1) {
	...
	if (typeof value === "string") {
		...
		if (resolved.format === "date-time" && !(DATE_TIME_PATTERN.test(value) && !Number.isNaN(Date.parse(value)))) {
