...
const defaultLauncher: WorkerLauncher = async (options) => {
	...
	const child = spawn(APP_NAME, args, { cwd: options.cwd, stdio: ["pipe", "ignore", "ignore"] });
...
export class BackgroundBus {
	...
	async spawn(options: { role: WorkerRole; prompt: string; model?: string; tools?: string[] }): Promise<WorkerRecord> {
		...
		if (
			wantsWrite &&
			[...this.workers.values()].some((worker) => worker.tools.includes("write") || worker.tools.includes("edit"))
		) {
	...
	async claim(agentId: string, path: string): Promise<void> {
		...
		if (holder !== undefined && holder !== agentId && this.get(holder) !== undefined)
			throw new Error(`Path already claimed: ${path}`);
		leases[path] = agentId;
	...
	private reap(): void {
		for (const [id, worker] of this.workers) {
			if (worker.launch.process !== undefined && worker.launch.process.exitCode !== null) this.workers.delete(id);
		}
	}
	...
	private async readLeases(): Promise<Record<string, string>> {
		...
		try {
			return JSON.parse(await readFile(this.leasesPath, "utf8")) as Record<string, string>;
		} catch (error) {
