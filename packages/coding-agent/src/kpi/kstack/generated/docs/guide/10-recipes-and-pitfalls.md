...
```text
/k-mode repro the duplicate write first. if there's a cheap test path, /tdd it. then fix and rerun.
```
...
- **Writing a `SKILL.md` freehand.** Route it through the [Authoring or modifying a skill playbook](../../skills/k-mode/playbooks/authoring-a-skill.md) so validation and review happen.
