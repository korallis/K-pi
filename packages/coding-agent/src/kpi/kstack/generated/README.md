...
**K-stack is my answer.** these are the same skills i use everyday to ship high quality code at Cursor. this turns cursor into a real engineering team. the goal is not to maximize loc, in fact it's the opposite. K-stack helps you write less, but higher quality code.

**K-stack gives you fearless parallelism.** when you can go deep on one agent and trust it to write good, verifiable code, you can truly parallelize with confidence. start multiple agents up with `k-mode` and trust that they'll apply rigorous engineering principles to their work.

**cursor gives you the best of all worlds.** every frontier model has its strengths and weaknesses. use any model with K-stack. in fact, many of my skills use multi-model workflows to take advantage of each model's unique strengths.
...
1. run [`/setup-kstack`](./skills/setup-kstack/SKILL.md) and choose which models you want.
2. use [`/k-mode`](./skills/k-mode/SKILL.md) whenever you're doing anything that requires rigor.

new here? the [K-stack guide](./docs/guide/README.md) walks you through a first real task, from setup and prompting through verification and overnight runs.

that's it. the other skills are situational; the mode skill uses them for you as needed. out of the box the mode splits work by model strength: precisely-specified code goes to sol, fast mechanical code goes to grok, and prose and judgment go to fable. the default panel is fable / sol / grok / opus 5. [`/setup-kstack`](./skills/setup-kstack/SKILL.md) changes any of it.

## usage

use [`/k-mode`](./skills/k-mode/SKILL.md) at the start of a task. it reads your request, picks from a set of playbooks, and runs the other skills as the steps need them.

### just use [`/k-mode`](./skills/k-mode/SKILL.md)
...
```
/k-mode this pr has a subtle bug where the scroll drifts every 750ms even when idle. repro
first, then fix and verify.
```

```
/k-mode i'm going to bed. land the stack even if ci flakes. i want everything merged by
morning.
```
...
| playbook | for |
|---|---|
| [investigation](./skills/k-mode/playbooks/investigation.md) | a read-only question. how does x work, why was y built this way, are we sure. |
| [bug fix](./skills/k-mode/playbooks/bug-fix.md) | reproduce a defect, root-cause it, and fix with runtime evidence. |
| [perf](./skills/k-mode/playbooks/perf-issue.md) | trace a measured slowness and improve it against a baseline. |
| [hillclimb](./skills/k-mode/playbooks/hillclimb.md) | sustained, scientific improvement of one metric against a target, looping hypotheses with before/after measurement and one commit per accepted win. |
| [runtime forensics](./skills/k-mode/playbooks/runtime-forensics.md) | diagnose a live symptom (leak, idle-cpu spin, glitch) from instrumentation. |
| [trace forensics](./skills/k-mode/playbooks/trace-forensics.md) | diagnose a captured profiling artifact (cpuprofile, trace, spindump, heap snapshot). |
| [feature](./skills/k-mode/playbooks/feature.md) | new or changed behavior, built from a named data shape. |
| [refactoring](./skills/k-mode/playbooks/refactoring.md) | a behavior-preserving change to structure or shape. |
| [prototype](./skills/k-mode/playbooks/prototype.md) | a throwaway sketch to make a design or behavioral decision cheaply, or to settle an empirical fork by observing it. |
| [visual parity](./skills/k-mode/playbooks/visual-parity.md) | pixel-exact ui equivalence between two implementations. |
| [authoring a skill](./skills/k-mode/playbooks/authoring-a-skill.md) | writing or editing a SKILL.md. |
| [eval](./skills/k-mode/playbooks/eval.md) | test how a skill or prompt change affects agent behavior, blinded. |
| [babysit](./skills/k-mode/playbooks/babysit.md) | drive a pr or a stack to merge-ready: conflicts, review threads, ci. |
| [autonomous run](./skills/k-mode/playbooks/autonomous-run.md) | drive a long task to completion without stopping. |
| [orchestrate](./skills/k-mode/playbooks/orchestrate.md) | a standing project handed to one coordinator chat: multi-day, many stacked prs, fleets of subagents. |
| [autopilot-full](./skills/k-mode/playbooks/autopilot-full.md) | run independent prs to merged with one owner per pr and root verification of each merge-ready head. |
| [session pickup](./skills/k-mode/playbooks/session-pickup.md) | resume or take over a prior agent's in-flight work. |
| [pause safely](./skills/k-mode/playbooks/pause-safely.md) | suspend in-flight work cleanly so it can be resumed later. |
| [multi-phase plan](./skills/k-mode/playbooks/multi-phase-plan.md) | work that spans phases or stacked PRs. |
| [worktree cleanup](./skills/k-mode/playbooks/worktree-cleanup.md) | reclaim disk by pruning merged or abandoned worktrees and stale ios simulators, safety-gated. |
...
2. matches your task to a [playbook](./skills/k-mode/playbooks/) and copies the steps in verbatim.
3. routes to the other skills as the steps fire.
4. writes unslopped replies framed for the consumer and the maintainer.

the full rules and playbooks live in [`skills/k-mode/SKILL.md`](./skills/k-mode/SKILL.md).

[`/k-mode`](./skills/k-mode/SKILL.md) is also a sticky mode: once entered it stays on across turns, applying itself when a playbook matches or the task needs rigor and staying out of the way otherwise. opt out any time by saying so.

[`/k-mode`](./skills/k-mode/SKILL.md) works extremely well with cursor's `/loop` command. you can make cursor work for many hours without sacrificing rigor.

## skills

[`/k-mode`](./skills/k-mode/SKILL.md) runs most of these for you when a step needs them (`how`, `why`, `architect`, `arena`, `swarm`, `interrogate`, `unslop`, `no-comments`, `technical-writing`, `tdd`, and the principles). the table below is for when you want one directly:
...
| skill | use it when |
|---|---|
| [`/k-mode`](./skills/k-mode/SKILL.md) | default entry point for any non-trivial task. |
...
| [`/setup-kstack`](./skills/setup-kstack/SKILL.md) | you want to pick which models K-stack uses per role. detects your models and writes a config rule. |
...
mostly i type [`/k-mode`](./skills/k-mode/SKILL.md) at the start of a task and let it route to a playbook. the other skills fire as the steps need them. a few i reach for directly.
...
```
bug fix:           /k-mode this pr has a subtle bug where the scroll drifts every 750ms even
                   when idle. repro first, then fix and verify.
perf:              /k-mode a big list takes a second or two to load even though we virtualize.
                   run a cpu trace and tell me why.
feature:           /k-mode build a small feature behind a feature flag. verify it really works.
prototype:         /k-mode build two prototypes of the markdown renderer so we can compare.
                   spawn an agent for each.
multi-phase:       /k-mode open source these skills as a plugin. nothing internal leaks, work
                   in a temp dir, show me the dependency graph first.
overnight run:     /k-mode i'm going to bed. land the stack even if ci flakes. i want
                   everything merged by morning.
babysit:           /k-mode check on pr 123. anything outstanding?
visual parity:     /k-mode the row spacing is too tall when this flag is on. the second image
                   is correct. repro and fix until it matches.
figure it out:     /k-mode i'm stepping away. migrate every caller from the synchronous store
...
## the `k-agent` and Comment Sicko subagents
...
twenty-one short skills, one principle each. `k-mode` indexes them inline and reads that index at task start. the standalone files are there so other skills can reference a principle by name, and so the index can point at the full rule for each.
...
a few things `k-mode` references but doesn't bundle:

- `/create-skill` is a cursor built-in. cursor also ships a built-in `/babysit`; inside `k-mode`, the [babysit playbook](./skills/k-mode/playbooks/babysit.md) supersedes it for pr-status requests.


## why are there no planning skills?

cursor already has a great plan mode which works great with K-stack. but personally, i don't believe in planning. the best spec is code. if you do want to make a plan, [`/k-mode`](./skills/k-mode/SKILL.md) covers it, but it's not a default. 

## make it yours

`k-mode` is my style. you may not want exactly that.

type [`/automate-me`](./skills/automate-me/SKILL.md). it mines your recent transcripts, drafts a `<your-name>-mode` skill from how you've actually worked, and routes through K-stack underneath. you keep K-stack as the base and end up with your own routing skill alongside `k-mode`.

models are configurable too. type [`/setup-kstack`](./skills/setup-kstack/SKILL.md). it detects the models you have access to and writes a small always-applied rule mapping each role (code, judgment, the review panels) to a model. every skill reads it and falls back to sensible defaults when the rule is absent, so you override only what you want.

## automations

K-stack also ships a dormant [benny automation pack](./automations/benny/). benny triages slack issue reports, then reproduces and fixes confirmed bugs with real ui evidence. its files are not registered as slash skills.

to set it up, point cursor at [`FOR_AGENTS.md`](./automations/benny/FOR_AGENTS.md). setup copies the pack into the target repository at `.cursor/automations/benny/`, enables K-stack there for shared skills, and keeps user configuration outside the copied pack.
