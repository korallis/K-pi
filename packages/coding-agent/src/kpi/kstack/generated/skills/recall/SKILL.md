...
Transcripts live at `~/.kpi/agent/sessions/<project>/<session>.jsonl`, where `<slug>` is the workspace path with the leading slash dropped and each "/" turned into "-" (the project slug is the workspace path with separators replaced by hyphens). Every line is one chat message.
