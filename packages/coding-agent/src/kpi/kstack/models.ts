...
export function createSuggestedModels(slugs: readonly string[]): KStackModels {
	...
	const families = slugs.filter(
		(slug, index) => slugs.findIndex((candidate) => candidate.split("/")[0] === slug.split("/")[0]) === index,
	);
...
export function assertKnownModels(document: KStackModels, slugs: readonly string[]): void {
	...
	for (const value of Object.values(document.roles).flatMap((entry) => (Array.isArray(entry) ? entry : [entry]))) {
...
export async function writeKStackModels(
	...
	path = join(getAgentDir(), "kstack", "models.json"),
): Promise<void> {
	...
	pi.registerCommand("setup-kstack", {
		...
		handler: async (_args, context) => {
			...
			if (slugs.length === 0) {
				...
				context.ui.notify(
					Object.entries(suggested.roles)
						.map(([role, slug]) => `${role} → ${Array.isArray(slug) ? slug.join(", ") : slug}`)
						.join("\n"),
					"info",
				);
