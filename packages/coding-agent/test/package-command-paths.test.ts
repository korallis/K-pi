...
const isUpstreamPi = APP_NAME === "pi";

/**
 * Upstream Pi's self-update cases. K-π rejects every self-update form before any
 * network or package-manager work (see the fork guard in package-manager-cli.ts),
 * so these are kept verbatim for the upstream merge and only run when the harness
 * identity is `pi` again.
 */
const itSelfUpdate = it.skipIf(!isUpstreamPi);

/** Cases that describe the fork guard itself, so they are meaningless for `pi`. */
const itFork = it.skipIf(isUpstreamPi);
...
`,
	...
	itFork("rejects every self-update form locally without any network or install work", async () => {
		...
		try {
			for (const args of [
				...
				const stderr = errorSpy.mock.calls.map(([message]) => String(message)).join("\n");
				expect(stderr, label).toContain(`error: ${APP_NAME} does not self-update.`);
				expect(stderr, label).toContain(`${APP_NAME} update --extensions`);
				expect(process.exitCode, label).toBe(1);
				const stdout = logSpy.mock.calls.map(([message]) => String(message)).join("\n");
				expect(stdout, label).not.toContain("Updated packages");
			}
	...
	itFork("documents update without a self-update target", async () => {
		const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
	...
	itSelfUpdate("allows explicit self-update checks when automatic version checks are disabled", async () => {
	...
	itSelfUpdate("retries a transient self-update version check", async () => {
		const previousSkipVersionCheck = process.env.PI_SKIP_VERSION_CHECK;
		delete process.env.PI_SKIP_VERSION_CHECK;
		const fetchMock = vi
			.fn()
	...
	itSelfUpdate("updates installer-managed Pi through a staged immutable release", async () => {
	...
	itSelfUpdate("rejects a concurrent managed update", async () => {
		const targetVersion = getNewerPatchVersion();
		const { managedRoot, npmRecordPath } = prepareManagedInstall(targetVersion);
	...
	itSelfUpdate("rejects forced managed reinstalls", async () => {
		const targetVersion = getNewerPatchVersion();
	...
	itSelfUpdate("keeps the managed release active when its update fails", async () => {
		const targetVersion = getNewerPatchVersion();
	...
	itSelfUpdate("keeps npm self-updates non-managed when the managed environment is inherited", async () => {
...
`,
	...
	itSelfUpdate("uses the current package name when the update check omits packageName", async () => {
		const globalPrefix = join(tempDir, "global-prefix");
...
`,
	...
	itSelfUpdate("installs the active package name from the update check during self-update", async () => {
		const globalPrefix = join(tempDir, "global-prefix");
		const selfPackageDir = join(globalPrefix, "lib", "node_modules", "@mariozechner", "pi-coding-agent");
		const fakeNpmPath = join(tempDir, "fake-npm.cjs");
		const recordPath = join(tempDir, "self-update.json");
...
`,
	...
	itSelfUpdate("prints a pnpm metadata hint when self-update fails", async () => {
	...
	itSelfUpdate("fails self-update when renamed npm package installation fails", async () => {
		const globalPrefix = join(tempDir, "global-prefix");
		const selfPackageDir = join(globalPrefix, "lib", "node_modules", "@mariozechner", "pi-coding-agent");
