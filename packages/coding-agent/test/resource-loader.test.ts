...
Project skill`,
	...
	pi.registerCommand("shared", {
		...
		handler: async () => {},
			...
			expect(extensionsResult.extensions[0].path).toBe(join(projectConfigDir, "extensions", "shared.ts"));
		});
...
Content`,
		...
		it("should skip project resources that require trust when project is not trusted", async () => {
			...
			const skillDir = join(projectConfigDir, "skills", "project-skill");
			const promptsDir = join(projectConfigDir, "prompts");
			const themesDir = join(projectConfigDir, "themes");
			mkdirSync(extensionsDir, { recursive: true });
			mkdirSync(skillDir, { recursive: true });
			mkdirSync(promptsDir, { recursive: true });
			mkdirSync(themesDir, { recursive: true });
			writeFileSync(join(projectConfigDir, "SYSTEM.md"), "Project system prompt.");
...
Project skill content`,
	...
	describe("system prompt sources", () => {
		it("exposes discovered project SYSTEM.md as the system prompt source", async () => {
			const systemPromptPath = join(projectConfigDir, "SYSTEM.md");
			mkdirSync(projectConfigDir, { recursive: true });
		...
		it("exposes discovered APPEND_SYSTEM.md as an append system prompt source", async () => {
			const appendSystemPromptPath = join(projectConfigDir, "APPEND_SYSTEM.md");
			mkdirSync(projectConfigDir, { recursive: true });
