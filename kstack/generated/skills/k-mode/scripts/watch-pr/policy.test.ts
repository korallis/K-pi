...
it("scans stacks tier-major so an uK-stack conflict outranks frontier CI", async () => {
  ...
  const uK-stack = await readSnapshot({
    reader: fakeReader({ facts: { mergeable: "CONFLICTING" } }),
    context: context(11),
    pendingHistory: "omit",
    allowDraft: false,
  });
  const decision = selectTierMajorStackDecision([frontier, uK-stack]);
...
it("attributes a stack wait to the PR whose checks are pending, not the bottom", async () => {
  ...
  const pendingUK-stack = await readSnapshot({
    reader: fakeReader({
      fastPath: { kind: "checks", checks: [pendingCheck("uK-stack-build")] },
    }),
    context: context(21),
    pendingHistory: "omit",
    allowDraft: false,
  });
  const decision = selectTierMajorStackDecision([readyBottom, pendingUK-stack]);
  expect(decision).toMatchObject({
    kind: "waiting",
    frontier: { number: 21 },
    pending: [{ name: "uK-stack-build" }],
  });
});
