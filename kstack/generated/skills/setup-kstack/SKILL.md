---
name: setup-kstack
description: Configure which models K-stack uses per role. Detects your available models and writes an always-applied rule that overrides the skill defaults. Use for /setup-kstack, "configure K-stack models", or changing K-stack's model choices.
---

# Setup K-stack

Write `~/.cursor/rules/K-stack-models.mdc`, an always-applied rule that sets K-stack's model per role. The skills read it and fall back to their inline defaults when a line is absent, so this is an override layer, not a requirement.
...
The default role-to-model mapping is the rule shape shown in step 5 below. If `~/.cursor/rules/K-stack-models.mdc` already exists, read it and treat its values as the current choices. Otherwise start from those defaults.
...
Write `~/.cursor/rules/K-stack-models.mdc` with `alwaysApply: true` and one line per role, using the same labels k-mode uses. Overwrite the whole file so re-runs stay idempotent. Shape:

```
---
description: K-stack per-role model choices (overrides skill defaults)
alwaysApply: true
---
# K-stack model configuration. One line per role. Delete a line to fall back to the skill default.
...
Check whether the project has a way to drive the real app for proof (a `verify-*` skill, or an existing harness). If not, offer once: "want a project-local verification skill, so agents can drive the app the way a user does and prove changes work? I can generate one with /create-verification-skill." On yes, invoke `/create-verification-skill` (resolves wherever K-stack is installed — workspace, user, or plugin). On no, move on without pushing.
