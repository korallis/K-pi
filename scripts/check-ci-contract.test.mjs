 ...
 * the scripts CI invokes, plus the two shipped workflows the guard requires —
 * `check.yml` and `release.yml`) with the workflow or script under test dropped
 * in. What the guard is *for* is which escalations it lets through, so accepted
 * cases assert zero violations and rejected cases assert the exact message — a
 * rule that fires for the wrong reason is a broken rule.
 */
...
const ASSEMBLED = {
	...
	nodeAuthToken: ["NODE", "AUTH", "TOKEN"].join("_"),
	npmPublish: ["npm", "publish"].join(" "),
	registryHost: ["registry", "npmjs", "org"].join("."),
	ghReleaseCreate: ["gh", "release", "create"].join(" "),
 ...
 * shipped check and release workflows are the baseline unless their omit flags
 * are set.
 */
function fixture(
	t,
	...
	if (!omitReleaseWorkflow) copyFileSync(workflowPath("release.yml"), join(workflowDirectory, "release.yml"));
...
/** The reviewed release workflow: publishes with OIDC and cuts the tag release. */
function releaseWorkflow({ extraStep = "", permissions = "      contents: write\n      id-token: write\n" } = {}) {
	return `name: release
on:
  push:
    tags: ["v*"]
...
${permissions}    steps:
      ...
      - uses: actions/setup-node@820762786026740c76f36085b0efc47a31fe5020 # v7
        with:
          ...
          registry-url: https://${ASSEMBLED.registryHost}
${extraStep}      - run: ${ASSEMBLED.npmPublish} --provenance --access public "$TARBALL"
      - run: ${ASSEMBLED.ghReleaseCreate} "$GITHUB_REF_NAME" "$TARBALL" --generate-notes --verify-tag
`;
}

/** The advisory reviewer: one comment, no other write. */
function aiReviewWorkflow({ extraStep = "", permissions = "      pull-requests: write\n" } = {}) {
...
${permissions}    steps:
      - uses: actions/checkout@3d3c42e5aac5ba805825da76410c181273ba90b1 # v7
        with:
          fetch-depth: 0
          persist-credentials: false
${extraStep}      - run: node scripts/ai-review.mjs
`;
}
...
test("baseline fixture with only the shipped check and release workflows is clean", (t) => {
...
test("the shipped ai-review workflow is accepted", (t) => {
	const root = fixture(t, { workflows: { "ai-review.yml": readFileSync(workflowPath("ai-review.yml"), "utf8") } });
	assert.deepEqual(inspectForkIntegrity(root), []);
});
...
test("release.yml may publish to the registry, cut a release and hold id-token", (t) => {
	const root = fixture(t, { omitReleaseWorkflow: true, workflows: { "release.yml": releaseWorkflow() } });
	assert.deepEqual(inspectForkIntegrity(root), []);
});

test("release.yml may not push", (t) => {
	const root = fixture(t, {
		omitReleaseWorkflow: true,
		workflows: { "release.yml": releaseWorkflow({ extraStep: "      - run: git push origin HEAD:refs/heads/main\n" }) },
	});
	assert.deepEqual(inspectForkIntegrity(root), [".github/workflows/release.yml: forbidden workflow that pushes"]);
});

test("release.yml may not hold a registry credential — trusted publishing is the whole point", (t) => {
	const root = fixture(t, {
		omitReleaseWorkflow: true,
		workflows: {
			"release.yml": releaseWorkflow({
				extraStep: `      - run: echo "\${{ secrets.${ASSEMBLED.nodeAuthToken} }}"\n`,
			}),
		},
	});
	assert.deepEqual(inspectForkIntegrity(root), [".github/workflows/release.yml: forbidden registry credential"]);
});

test("release.yml may not widen past contents and id-token", (t) => {
	const root = fixture(t, {
		omitReleaseWorkflow: true,
		workflows: {
			"release.yml": releaseWorkflow({
	...
	assert.deepEqual(inspectForkIntegrity(root), [
		'.github/workflows/release.yml: forbidden write permission "pull-requests: write"',
	]);
});

test("publishing outside release.yml is rejected", (t) => {
	const root = fixture(t, {
		workflows: {
			"nightly.yml": readOnlyWorkflow(`    steps:\n      - run: ${ASSEMBLED.npmPublish} --access public ./x.tgz\n`),
		},
	});
	assert.deepEqual(inspectForkIntegrity(root), [
		`.github/workflows/nightly.yml: forbidden ${ASSEMBLED.npmPublish} outside release.yml`,
	]);
});

test("the advisory reviewer may post its comment", (t) => {
	assert.deepEqual(inspectForkIntegrity(fixture(t, { workflows: { "ai-review.yml": aiReviewWorkflow() } })), []);
});

test("the advisory reviewer may not gain contents write", (t) => {
	const root = fixture(t, {
		workflows: {
			"ai-review.yml": aiReviewWorkflow({ permissions: "      contents: write\n      pull-requests: write\n" }),
		},
	});
	assert.deepEqual(inspectForkIntegrity(root), [
		'.github/workflows/ai-review.yml: forbidden write permission "contents: write"',
	]);
});

test("the advisory reviewer inherits no rule exemption", (t) => {
	const root = fixture(t, {
		workflows: {
			"ai-review.yml": aiReviewWorkflow({
				extraStep: `      - run: ${ASSEMBLED.npmPublish} --access public ./x.tgz\n      - run: git push origin HEAD\n`,
			}),
		},
	});
	assert.deepEqual(inspectForkIntegrity(root), [
		`.github/workflows/ai-review.yml: forbidden ${ASSEMBLED.npmPublish} outside release.yml`,
		".github/workflows/ai-review.yml: forbidden workflow that pushes",
	]);
});
...
test("shipped check workflow requires same-repository PR heads on self-hosted", () => {
	...
	assert.match(check, /github\.event\.pull_request\.head\.repo\.full_name\s*==\s*github\.repository/);
	...
	assert.match(check, /maintainer-owned branch/i);
});
...
jobs:
  check:
    ...
    steps:
      - run: echo ok
`;
	const root = fixture(t, { omitCheckWorkflow: true, workflows: { "check.yml": broken } });
	assert.deepEqual(inspectForkIntegrity(root), [
...
test("auto-merge may not bypass the required check with --admin", (t) => {
	const root = fixture(t, {
...
test("wrapped gh pr merge --admin is rejected", (t) => {
	const root = fixture(t, {
		workflows: { "auto-merge.yml": autoMergeWorkflow('gh pr merge --auto \\\n        --admin --merge "$PR_URL"') },
	});
	assert.ok(inspectForkIntegrity(root).some((v) => v.includes("--admin")));
});

test("comment-only --auto does not satisfy merge wait-for-checks", (t) => {
	const root = fixture(t, { workflows: { "auto-merge.yml": autoMergeWorkflow('gh pr merge --squash "$PR_URL" # --auto') } });
	assert.ok(inspectForkIntegrity(root).some((v) => v.includes("without --auto")));
});
...
test("third-party release actions are rejected even inside an approved workflow", (t) => {
	const root = fixture(t, {
		omitReleaseWorkflow: true,
		workflows: {
			"release.yml": releaseWorkflow({
	...
	assert.deepEqual(inspectForkIntegrity(root), [".github/workflows/release.yml: forbidden release automation"]);
});

test("GitHub release commands are rejected outside release.yml", (t) => {
	const root = fixture(t, {
		workflows: { "nightly.yml": readOnlyWorkflow("    steps:\n      - run: gh release edit v1 --draft=false\n") },
	});
	assert.deepEqual(inspectForkIntegrity(root), [
		".github/workflows/nightly.yml: forbidden GitHub release automation",
	]);
});

test("registry credentials are rejected outside release.yml too", (t) => {
	const root = fixture(t, {
		workflows: {
			"nightly.yml": readOnlyWorkflow(`    steps:\n      - run: echo "${ASSEMBLED.registryCredential} unused"\n`),
		},
	});
	assert.deepEqual(inspectForkIntegrity(root), [".github/workflows/nightly.yml: forbidden registry credential"]);
});

test("a registry target is rejected inside a non-release approved workflow", (t) => {
	const root = fixture(t, {
		workflows: {
			"auto-merge.yml": autoMergeWorkflow('gh pr merge --auto --merge "$PR_URL"', {
...
test("every retired workflow filename is rejected", (t) => {
	for (const name of ["cursor-review.yml", "grok-review.yml", "react-doctor.yml"]) {
		const root = fixture(t, { workflows: { [name]: readOnlyWorkflow("    steps:\n      - run: echo old\n") } });
		assert.deepEqual(inspectForkIntegrity(root), [`.github/workflows/${name}: retired workflow must not exist`]);
	}
});

test("upstream workflow filenames are rejected on sight", (t) => {
	for (const name of ["ci.yml", "npm-audit.yml", "publish-model-catalog.yml", "approve-contributor.yml"]) {
		const root = fixture(t, { workflows: { [name]: readOnlyWorkflow("    steps:\n      - run: echo hi\n") } });
		assert.deepEqual(inspectForkIntegrity(root), [
			`.github/workflows/${name}: upstream workflow filename must not be imported`,
		]);
	}
});
...
test("a root script may not hold a registry credential either", (t) => {
	const root = fixture(t, {
		scripts: { "pack.mjs": `#!/usr/bin/env node\nconst t = process.env.${ASSEMBLED.nodeAuthToken};\n` },
	});
	assert.deepEqual(inspectForkIntegrity(root), ["scripts/pack.mjs: forbidden registry credential"]);
});

test("losing the check gate workflow is a violation", (t) => {
	assert.deepEqual(inspectForkIntegrity(fixture(t, { omitCheckWorkflow: true })), [
		".github/workflows/check.yml: required workflow is missing (nothing runs the gate or the release)",
	]);
});

test("losing the release workflow is a violation", (t) => {
	assert.deepEqual(inspectForkIntegrity(fixture(t, { omitReleaseWorkflow: true })), [
		".github/workflows/release.yml: required workflow is missing (nothing runs the gate or the release)",
	]);
});

test("a script CI invokes must exist as a root npm script", (t) => {
	const manifest = cleanManifest();
	delete manifest.scripts.pack;
	assert.deepEqual(inspectForkIntegrity(fixture(t, { manifest })), [
		'package.json: missing required script "pack" (CI invokes it)',
	]);
});

test("a manifest may not point at a script file that does not exist", (t) => {
	const manifest = cleanManifest();
	manifest.scripts.ship = "node scripts/does-not-exist.mjs";
	assert.deepEqual(inspectForkIntegrity(fixture(t, { manifest })), [
		'package.json: script "ship" runs scripts/does-not-exist.mjs, which does not exist',
	]);
});

test("write-all is rejected even on an otherwise empty workflow", (t) => {
	const root = fixture(t, {
		workflows: { "nightly.yml": readOnlyWorkflow("    permissions: write-all\n    steps:\n      - run: echo hi\n") },
	});
	assert.deepEqual(inspectForkIntegrity(root), [
...
test("quoted write-all is rejected", (t) => {
	const root = fixture(t, {
		workflows: { "nightly.yml": readOnlyWorkflow('    permissions: "write-all"\n    steps:\n      - run: echo hi\n') },
	});
	assert.deepEqual(inspectForkIntegrity(root), [
...
test("comment mention of pull_request_target is allowed", (t) => {
	const root = fixture(t, {
		workflows: {
			"nightly.yml": readOnlyWorkflow("    # this fork refuses pull_request_target\n    steps:\n      - run: echo hi\n"),
		},
	});
	assert.deepEqual(inspectForkIntegrity(root), []);
});
...
test("the CLI exits 0 and says so on a clean repository", (t) => {
	...
	assert.match(result.stdout, /10 required scripts present/);
	assert.match(result.stdout, /3 reviewed write exemptions/);
});
...
test("the CLI exits 1 and names the violation and the reviewed exemptions", (t) => {
	...
	assert.match(result.stderr, /release\.yml: publishes @korallis\/k-pi with npm trusted publishing \(OIDC\)/);
	assert.match(result.stderr, /auto-merge\.yml: asks GitHub to merge only after required checks pass/);
	assert.match(result.stderr, /ai-review\.yml: posts one advisory review comment/);
});
