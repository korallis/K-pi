export const meta = {
  name: 'kpi-wave1-implement',
  description: 'Implement wave 1 of the K-π operator fixes: anthropic-auth, plan-gate, agents-visibility on disjoint files, each verified adversarially and fixed once',
  phases: [
    { title: 'Implement', detail: 'three implementers in parallel, one writer per file' },
    { title: 'Verify', detail: 'one adversarial verifier per package runs the scoped tests and reads the diff' },
...
const PACKAGES = [
  {
    ...
    owned: [
      ...
      'test/accounts-errors.test.ts (create)',
...
- Rebuilding dist for the harness test is the lead's job after the wave.`,
  },
  {
    ...
    owned: [
      'packages/coding-agent/src/kpi/extensions/bus/sessions-snapshot.ts (create)',
      'packages/coding-agent/src/kpi/extensions/bus/sessions-command.ts (create)',
      'packages/coding-agent/src/kpi/extensions/bus/spawn.ts',
      'packages/coding-agent/src/kpi/extensions/bus/communicate.ts',
      'test/bus.test.ts',
    ],
    decisions: `
- WAVE SCOPE (integration plan): in this wave you land ONLY the bus half: sessions-snapshot.ts (new, the sole liveness registry, exactly the exported shapes in the design), sessions-command.ts (new, the /agents command and formatSessionsTable), spawn.ts (LiveWorker, liveWorkers(), countLiveProcesses derived from it, node?: string on spawn options/WorkerRecord/WorkerStatus), communicate.ts (registerLiveBus on activeBus, releases at session_shutdown, registerSessionsCommand in registerParentTools) and test/bus.test.ts.
- Do NOT delete bus/live-snapshot.ts and do NOT edit control-plane.ts, board.ts, graph/engine.ts, gated-loop.ts, test/graph-engine.test.ts, test/reviewer-session.test.ts, test/operator-ui.test.ts, test/control-plane.test.ts: those hunks belong to the board package in a later wave. In communicate.ts KEEP the existing setLiveWorkerCountProvider bridge but point it at the new registry: setLiveWorkerCountProvider(() => liveWorkerCount()) importing liveWorkerCount from ./sessions-snapshot.ts (so control-plane's old import keeps counting parent-tool workers until the board package removes it). Put every hunk you are not allowed to land (engine brackets, gated-loop option literals, board agentsCell + BoardModel.sessions, control-plane buildBoardModel derivation, the four handed-over tests) VERBATIM under handover.hunks_for_other_owners with owner "board-live".
- sessions-command.ts must import from board.ts only for truncatePlain if it exists there (check); if it does not, implement a local plain truncation and say so.
- The /agents handler: sessionsSnapshot({ main: { sessionId, model, pid: process.pid } }) with no jobId (list every session this process owns); job status line from readLiveJob; the one failure path (run store unreadable) reported as an error notify starting with "K-π /agents could not read the run store: ".`,
  },
]
...
phase('Implement')
log('Wave 1: implementing anthropic-auth, plan-gate, agents-visibility in parallel')
