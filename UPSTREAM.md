...
The register tracks upstream-owned paths only. `src/kpi/**`, root `test/**`, `docs/**`, `design/**`, `fixtures/**`, and the new root files (`UPSTREAM.md`, `NOTICE`, `START-HERE.md`, `upstream.json`, CI guard scripts) are K-π-owned and never conflict; they are not listed here. RP-22's native seams below are included alongside the existing base-diff register; scoped local proof and built startup do not establish live provider quality or completion of the full migration mandate.

**Fork identity — `packages/coding-agent/` (high risk: expect a conflict per release; K-π always wins)**

| File | Patch |
|---|---|
| `package.json` | The fork's own version (decoupled from upstream `0.84.x`; the `v<version>` release tag must equal it). `bin` is exactly `kpi` + `k-pi`; the `pi` bin is gone. `piConfig` is `{ name: "kpi", title: "K-π", configDir: ".kpi" }`. Publish metadata is removed. The build cleans `dist` first and copies K-π's runtime assets into `dist/kpi`. |
| `CHANGELOG.md` | **Replaced by K-π's own release history**; upstream's file is kept verbatim as `CHANGELOG.upstream.md` so its history still merges. The interactive startup compares the entries against `lastChangelogVersion` = K-π's version (`0.x`), so upstream's `0.84.x` entries read as "new" on every launch after the first and the whole upstream changelog was printed each start. On an upstream merge, take theirs into `CHANGELOG.upstream.md` and keep ours. |
...
| `src/core/model-runtime.ts` | RP-22 adds a session-owned request-auth resolver and per-request override, including availability checks. The selected pooled grant resolves before native provider auth/request shaping; explicit denial fails closed, and absent policy delegates to native auth. This prevents an unrelated primary grant from deciding a pooled request's protocol identity. Keep provider conversion and OAuth semantics native on merge. |
| `src/core/sdk.ts` | RP-22 binds `before_provider_auth` into native session construction and passes a request-local resolver and request ID into `streamSimple`, correlating credential selection with response accounting. The existing native context transform runs the canonical-context extension before inference; do not bypass it during retries/continuation or replace this seam with header-only scheduling. |
| `src/core/extensions/types.ts` | RP-22 declares `BeforeProviderAuthEvent` and its subscription (`model`, availability `checkOnly`, request ID and optional selected/denied auth), plus explicit `ContextEventResult.block/reason`. Credential and mandatory-context refusal must remain expressible without being downgraded to an extension diagnostic. |
| `src/core/extensions/runner.ts` | RP-22 dispatches pre-auth handlers without swallowing failures and returns the selected auth result. Explicit context refusal throws outside the ordinary handler-error diagnostic path, so missing mandatory context prevents inference rather than continuing silently. Ordinary nonblocking extension errors retain upstream handling. |
| `src/core/extensions/index.ts` | RP-22 exports `BeforeProviderAuthEvent` through the native extension barrel; keep the public type aligned with the runner and subscription API. |
| `src/index.ts` | RP-22 exports `BeforeProviderAuthEvent` from the package API so SDK/extension consumers can use the native pre-auth contract without importing private paths. |
| `src/modes/interactive/interactive-mode.ts` | Subscription OAuth for a K-π pool delegates `/login` to the built-in pooled account command; non-pool OAuth and API-key login keep upstream behavior. RP-22 also changes the actual terminal startup helper text from Pi to K-π so the built interactive surface does not advertise the upstream product. |
| `src/modes/interactive/components/login-dialog.ts` | Preserves native clickable OAuth links and adds exact-URL Ctrl+Y copying without altering pending callback input; explains remote-browser callback completion. |
| `src/utils/open-browser.ts` | Skips the native desktop launcher under SSH/Mosh; opening a remote server's browser does not open the operator's client browser. Local shell-free launch behavior is unchanged. |
| `src/utils/clipboard.ts` | SSH/Mosh copies go directly to the terminal via OSC 52, never the server's native desktop clipboard. Local clipboard backends retain their existing fallback order. |
| `test/{open-browser,clipboard}.test.ts`, `test/suite/regressions/5433-extension-oauth-prompt-input.test.ts` | Regressions for SSH browser suppression, exact clipboard payload without server clipboard side effects, and native login copying without corrupting partially entered callback text. |
...
| `test/extensions-runner.test.ts` | RP-22 adds a regression proving explicit mandatory-context refusal rejects native context emission rather than being swallowed as a diagnostic. `.kpi/proof/RP-22/native-context-runner.json` records the scoped suite passing; it is not live provider inference evidence. |
...
| `test/kpi-plan-stack-response.test.ts` | K-π stack-response regressions. Removed the retired rule that rejected an otherwise valid feature merely because its folder was named `api`; protected bounds and ownership remain checked by the root K-π suite. |
...
| `packages/evals/package.json` | Workspace dependency range follows the fork version (`^<fork version>`, including a release-candidate suffix) instead of upstream `^0.84.4`; update with the coding-agent version so npm resolves the owned workspace rather than an unpublished registry version. |
| `AGENTS.md`, `README.md`, `packages/coding-agent/README.md` | Root docs replaced by K-π's authority docs; the package README keeps upstream's reference body under a fork banner and first-party installation/Cursor instructions (low risk — regenerate the banner side, take upstream's body updates). |
