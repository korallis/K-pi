...
const POLICY_TEMPLATE = new URL("../templates/policy.json", import.meta.url);
...
function protectedRunArtifact(
  cwd: string,
  path: string,
): ProtectedRunArtifact | undefined {
  const relativePath = relativeWritePath(cwd, path);
  if (
    relativePath === undefined ||
    !/^\.pi\/runs\/[^/]+\//u.test(relativePath)
  ) {
    return undefined;
  }
  if (relativePath.endsWith("/verdict.json")) {
    return "verdict.json";
  }
  return relativePath.endsWith("/release.approved")
    ? "release.approved"
    : undefined;
}
...
export function isWriteAllowed(
  ...
  if (
    relativePath === undefined ||
    relativePath.split("/").some((part) => SENSITIVE_FILE_PATTERN.test(part))
  ) {
    return false;
  }
  ...
  return writeAllow.some((allowedPath) => {
    const normalizedAllowedPath = normalizePath(
      isAbsolute(allowedPath)
        ? relative(resolve(cwd), resolve(allowedPath))
        : allowedPath,
    );
...
export async function ensurePolicyFile(cwd: string): Promise<string> {
  ...
  const policyPath = resolve(piDirectory, "policy.json");
  await mkdir(piDirectory, { recursive: true });
  try {
    await copyFile(POLICY_TEMPLATE, policyPath, constants.COPYFILE_EXCL);
  } catch (error) {
...
export function registerPolicy(
  ...
  pi.on("tool_call", async (event, ctx) => {
    const [policy, writeAllow] = await Promise.all([
      readPolicy(ctx.cwd),
      options.resolveWriteAllow?.(ctx.cwd) ?? [],
    ]);
