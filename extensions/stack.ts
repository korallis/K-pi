...
export function assertDuneStack(stack: DuneStack): void {
  if (stack.version !== 1 || stack.shape !== "dune" || stack.scaffold_first !== true) throw new Error("Invalid Dune stack header");
  if (stack.delivery === "horizontal" && !stack.delivery_reason?.trim()) throw new Error("Horizontal delivery requires a reason");
  for (const module of stack.modules) {
    if (basename(normalize(module.folder)) !== module.id) throw new Error(`Module folder must match id: ${module.id}`);
    if (GENERIC.has(module.id) && module.purpose.trim().split(/\s+/u).length < 3) throw new Error(`Generic module needs a tight purpose: ${module.id}`);
    const folder = `${normalize(module.folder)}${sep}`;
    if (!normalize(module.interface).startsWith(folder)) throw new Error(`Interface must live inside ${module.folder}`);
    if (module.id === "auth" && /(?:^|\/)(?:lib|services)(?:\/|$)/u.test(module.folder)) throw new Error("Auth must live in its auth folder");
    if (!module.allowed_paths.some((path) => path.startsWith(`${module.folder}/`))) throw new Error(`Module ${module.id} does not allow its own folder`);
    if (!module.allowed_paths.some((path) => path.startsWith(`test/${module.id}/`))) throw new Error(`Module ${module.id} lacks its test twin`);
  }
}
...
export function assertClaimInModule(projectRoot: string, path: string, module: StackModule): void {
  ...
  if (projectPath.startsWith("..") || !module.allowed_paths.some((pattern) => projectPath.startsWith(globPrefix(pattern)))) {
