...
export type EventInput =
  ...
  | Event<
      "ac.refused",
      { quality: "executable" | "partial" | "narrative"; reason: string }
    >
...
export function redactJson(value: JsonValue): JsonValue {
  ...
  if (value !== null && typeof value === "object") {
    ...
    for (const [key, child] of Object.entries(value)) {
      redacted[redactString(key)] = SENSITIVE_KEY_PATTERN.test(key)
        ? REDACTED
        : redactJson(child);
    }
    return redacted;
...
function assertPayloadAllowed(value: JsonValue, path: string): void {
  ...
  for (const [key, child] of Object.entries(value)) {
    ...
    if (
      REJECTED_KEYS[normalized] === true ||
      (isContainer && REJECTED_CONTAINER_KEYS[normalized] === true)
    ) {
