...
export function registerBackgroundBus(pi: ExtensionAPI, launcher?: WorkerLauncher): void {
  ...
  pi.registerTool(defineTool({
    ...
    description: "Start one local background Pi worker; at most two workers and one writer",
    parameters: Type.Object({
      role: Type.Union([Type.Literal("implementer"), Type.Literal("reviewer"), Type.Literal("tester"), Type.Literal("arena"), Type.Literal("explorer")]),
      prompt: Type.String(),
  ...
  pi.registerTool(defineTool({
    name: "communicate",
    label: "Communicate",
    description: "Send steering or follow-up input to a background Pi worker",
    parameters: Type.Object({
      to: Type.String(),
      message: Type.String(),
