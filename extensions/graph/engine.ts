...
export type GraphAgentSessionFactory = (
  options: CreateAgentSessionOptions,
) => Promise<{ session: GraphAgentSession }>;
...
function assertStatePath(path: string, label: string): void {
  ...
  if (
    parts.some(
      (part) => part.length === 0 || FORBIDDEN_PATH_PARTS.has(part),
    )
  ) {
...
function validateNode(value: unknown, index: number): asserts value is GraphNode {
  ...
  if (value.readOnly) {
    ...
    if (mutatingTool !== undefined) {
      throw new Error(
        `read-only agent node ${value.id} cannot enable tool ${mutatingTool}`,
      );
    }
  }
  if (value.response !== undefined) {
    ...
    if (
      isAbsolute(value.response.path) ||
      value.response.path.split(/[\\/]/u).some((part) => part === "..")
    ) {
    ...
    for (const [statePath, responsePath] of Object.entries(value.response.state)) {
      ...
      assertString(
        responsePath,
        `agent node ${value.id}.response.state.${statePath}`,
      );
...
export function validateGraphDefinition(
  value: unknown,
): asserts value is GraphDefinition {
  if (!isJsonObject(value)) {
...
export async function loadNamedGraph(
  projectRoot: string,
  name: string,
  ...
  const projectPath = join(projectRoot, ".pi", "graphs", fileName);
  try {
  ...
  return loadGraph(new URL(`../../graphs/${fileName}`, import.meta.url));
}
...
async function loadResponseSchema(
  ...
  const projectPath = join(projectRoot, ".pi", "schemas", name);
  try {
...
export class GraphEngine {
    ...
    this.runState = initialState ?? {
      ...
      nodes: Object.fromEntries(
        graph.nodes.map((node) => [
          node.id,
          { status: "pending" as const, runs: 0 },
        ]),
      ),
    };

    if (
      this.runState.graphId !== graph.id ||
      this.runState.jobId !== options.jobId
    ) {
  ...
  private async writeCheckpoint(): Promise<void> {
    ...
    await atomicWrite(
      join(this.checkpointDirectory(), name),
  ...
  private nextNodes(nodeIds: readonly string[], values: JsonObject): string[] {
    ...
    for (const nodeId of nodeIds) {
      for (const edge of this.outgoing(nodeId)) {
        if (
          edge.when === undefined ||
          isDeepStrictEqual(getStatePath(values, edge.when.path), edge.when.equals)
        ) {
          targets.add(edge.to);
        }
      }
  ...
  private async createSessionForNode(
    ...
    const unexpectedTool = node.readOnly
      ? result.session
          .getActiveToolNames()
          .find((tool) => !node.tools.includes(tool))
      : undefined;
  ...
  private async executeNode(node: GraphNode): Promise<NodeResult> {
    ...
    try {
      ...
      for (let attempt = 0; attempt <= node.response.retries; attempt += 1) {
        ...
        for (const [statePath, responsePath] of Object.entries(
          node.response.state,
        )) {
          const value = getStatePath(output, responsePath);
          if (value === undefined) {
            validationErrors = [
              `response state path ${responsePath} does not exist`,
            ];
            break;
          }
        ...
        await atomicWrite(
          join(this.runDirectory(), node.response.path),
  ...
  async runSuperstep(): Promise<Readonly<GraphRunState>> {
    ...
    if (this.runState.active.length > this.graph.limits.maxConcurrency) {
      return this.fail(
        "ready nodes exceed maxConcurrency",
    ...
    if (humanNode !== undefined) {
      if (activeNodes.length !== 1 || humanNode.type !== "human") {
        return this.fail(
          "human nodes cannot share a superstep",
          this.runState.active,
        );
      }
    ...
    for (const node of activeNodes) {
      ...
      if (nodeState.runs >= this.graph.limits.maxNodeRuns) {
        return this.fail(
          `node ${node.id} exceeded maxNodeRuns`,
          [node.id],
        );
      }
      nodeState.runs += 1;
    ...
    try {
      results = await Promise.all(
        activeNodes.map((node) => this.executeNode(node)),
      );
    } catch (error) {
    ...
    for (const result of results) {
      for (const [path, value] of Object.entries(result.assignments)) {
        if (seenPaths.has(path)) {
          return this.fail(
            `multiple nodes wrote state path ${path} in one superstep`,
            this.runState.active,
          );
        }
        seenPaths.add(path);
    ...
    this.runState.status =
      this.runState.active.length === 0 ? "completed" : "running";
    this.runState.superstep += 1;
    await this.writeCheckpoint();
    return this.runState;
  }
  ...
  static async restore(
    ...
    const checkpointNames = (await readdir(directory))
      .filter((name) => /^checkpoint-\d{6}\.json$/.test(name))
      .sort();
