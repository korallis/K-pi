...
function scoreCompiledCriteria(
  criteria: readonly CompiledAcceptanceCriterion[],
): AcceptanceScore {
  ...
  const missingChecks = requiredCriteria.flatMap((criterion) => {
    ...
    return missing.length === 0
      ? []
      : [{ id: criterion.id, statement: criterion.statement, missing }];
  });
...
export function compileAcceptanceCriteria(input: string): AcceptanceCompilation {
  const acceptance: CompiledAcceptanceCriterion[] = acceptanceStatements(
    input,
  ).map(({ id, statement }) => ({
    id,
    statement,
...
export function scoreAcceptanceCriteria(
  input: string | readonly CompiledAcceptanceCriterion[],
): AcceptanceScore {
