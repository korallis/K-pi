...
export async function conductResearch(
  ...
  if (document === undefined && dependencies.perplexityKey !== undefined) {
    try {
      const results = await perplexitySearch(task.goal, dependencies.perplexityKey, { maxResults: 5, fetch: dependencies.fetch });
      const resultSources = sources(results);
      if (resultSources.length >= 2) {
  ...
  const markdown = [
    ...
    "",
    ...document.sources.flatMap((source) => [`- [${source.title}](${source.url})`, source.excerpt === undefined ? "" : `  ${source.excerpt.replaceAll("\n", " ")}`]),
  ].filter(Boolean).join("\n");
...
export async function assertResearchFresh(runDirectory: string, task: Task): Promise<void> {
  ...
  if (typeof document !== "object" || document === null || !("task_hash" in document) || document.task_hash !== taskHash(task)) {
