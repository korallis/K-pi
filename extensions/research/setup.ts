...
async function readSecrets(path: string): Promise<Record<string, unknown>> {
  try {
    ...
    if (typeof value !== "object" || value === null || Array.isArray(value)) throw new Error("Account secrets must be an object");
    return { ...value };
  } catch (error) {
...
export async function saveResearchKeys(
  ...
  path = join(homedir(), ".pi", "agent", "accounts.secrets.json"),
): Promise<void> {
...
export async function promptResearchSetup(context: ExtensionCommandContext): Promise<void> {
  ...
  const values = {
    ...
    perplexity: perplexity === undefined || perplexity.trim() === "" || perplexity.trim().toLowerCase() === "s" ? undefined : perplexity.trim(),
  };
  if (values.exa !== undefined || values.perplexity !== undefined) await saveResearchKeys(values);
  context.ui.notify(values.exa !== undefined || values.perplexity !== undefined ? "External research configured" : "Research mode: local", "info");
}
