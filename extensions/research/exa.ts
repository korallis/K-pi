...
function normalize(value: unknown): ResearchResult[] {
  ...
  return entries.slice(0, 10).flatMap((entry) => {
    ...
    const record = entry;
    const rawHighlights: unknown =
      "highlights" in record ? record.highlights : undefined;
...
async function request(path: string, key: string, body: unknown, signal?: AbortSignal, fetchImpl: typeof fetch = fetch): Promise<ResearchResult[]> {
...
export function exaSearch(query: string, key: string, options: { numResults?: number; signal?: AbortSignal; fetch?: typeof fetch } = {}): Promise<ResearchResult[]> {
  return request("search", key, {
    query,
    type: "auto",
    numResults: capped(options.numResults),
    contents: { highlights: true },
  }, options.signal, options.fetch);
}

export function exaContents(urls: readonly string[], key: string, options: { signal?: AbortSignal; fetch?: typeof fetch } = {}): Promise<ResearchResult[]> {
