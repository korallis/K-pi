...
async function readStateCandidate(
  directory: string,
): Promise<(ActiveJob & { modifiedAt: number }) | undefined> {
  ...
  try {
    ...
    const jobId =
      typeof state.job_id === "string" ? state.job_id : basename(directory);
    return {
      directory,
...
export async function readActiveJob(cwd: string): Promise<ActiveJob | undefined> {
  const runsDirectory = join(cwd, ".pi", "runs");
  let entries;
  try {
...
export async function createStatusWidget(cwd: string): Promise<string[]> {
  ...
  const maxRounds = numberValue(
    job.state.maxRounds ?? nestedValue(job.state, "limits", "maxRounds"),
    3,
  );
...
async function installWidget(ctx: ExtensionContext): Promise<boolean> {
  ...
  if (typeof ctx.ui.setTheme === "function") {
    ctx.ui.setTheme(
      job?.state.status === "APPROVAL" || job?.state.graph_status === "interrupted"
        ? "protocol-blue"
        : "loop-amber",
    );
  }
...
async function handleKpiCommand(
  ...
  try {
    ...
    let outcome;
    try {
      await readFile(join(ctx.cwd, ".pi", "runs", command, "task.json"), "utf8");
