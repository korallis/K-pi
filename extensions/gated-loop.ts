...
import { isJsonObject, type GraphRunState } from "./graph/schema.ts";
...
export const CONVENTIONAL_COMMIT_PATTERN =
  /^(feat|fix|docs|refactor|test|chore)(\(.+\))?: /u;
...
async function snapshotPlan(
  ...
  const sources = metadata.isDirectory()
    ? PLAN_FILES.map((name) => join(source, name))
    : [source];
...
async function writePlanSnapshot(
  runDirectory: string,
  plan: PlanSnapshot,
): Promise<void> {
  for (const file of plan.files) {
  ...
  const fingerprints = Object.fromEntries(
    plan.files.map((file) => [file.destination, file.fingerprint]),
  );
  await atomicWrite(
    join(runDirectory, "fingerprints.json"),
    `${JSON.stringify({ plan: fingerprints }, null, 2)}\n`,
  );
}
...
function stateDocument(
  ...
  return {
    ...
    stage: stageFor(node),
    node,
    passed: isJsonObject(state.values.test)
      ? state.values.test.passed
      : undefined,
...
async function writeState(
  ...
  await atomicWrite(
    ...
    `${JSON.stringify(
      stateDocument(task, state, round, terminalStatus, reason),
      null,
      2,
    )}\n`,
  );
...
async function createJobBranch(
  projectRoot: string,
  jobId: string,
): Promise<void> {
...
async function worktreeSnapshot(
  ...
  for (const path of paths) {
    if (path === ".pi/runs" || path.startsWith(".pi/runs/")) {
      continue;
    }
    try {
...
function reviewFingerprint(
  ...
  return typeof review.output_fingerprint === "string"
    ? review.output_fingerprint
    : undefined;
}
...
function reviewStatus(state: Readonly<GraphRunState>): string | undefined {
  const review = state.values.review;
  return isJsonObject(review) && typeof review.status === "string"
    ? review.status
    : undefined;
}
...
function evidencePasses(task: Task, evidence: unknown): boolean {
  ...
  for (const command of commands) {
    if (
      ...
      typeof command.exit === "number"
    ) {
      latestCommandExit.set(command.cmd, command.exit);
    }
  }
  if (
    latestCommandExit.size === 0 ||
    [...latestCommandExit.values()].some((exit) => exit !== 0)
  ) {
    return false;
  }
  ...
  return task.acceptance
    ...
    .every((criterion) => passedIds.has(criterion.id));
}
...
async function driveUntilPause(
  ...
  while (state.status === "running") {
    ...
    if (
      state.active.includes("release.set") &&
      !(await releaseReady(state, projectRoot, jobDirectory, task))
    ) {
      return {
        state,
        stopState: currentStopState,
...
export async function resumeLoop(
  ...
  const jobDirectory = join(ctx.cwd, ".pi", "runs", jobId);
  const task = JSON.parse(
    await readFile(join(jobDirectory, "task.json"), "utf8"),
  ) as Task;
  const stateDocument = JSON.parse(
    await readFile(join(jobDirectory, "state.json"), "utf8"),
  ) as Record<string, unknown>;
  if (stateDocument.status === "DONE") {
    return { jobId, status: "DONE" };
  }
  const graph = await loadNamedGraph(
    ctx.cwd,
    task.mode === "autopilot" ? "coding-loop.auto" : "coding-loop.gated",
  );
  ...
  const baselineSource = JSON.parse(
    await readFile(join(jobDirectory, "baseline.json"), "utf8"),
  ) as Record<string, string>;
  const baseline = new Map(Object.entries(baselineSource));
  const eventsPath = join(jobDirectory, "events.jsonl");
  const storedRound =
    typeof stateDocument.round === "number" ? stateDocument.round : 0;
...
export async function runLoop(
  ...
  const plan =
    invocation.planPath === undefined
      ? undefined
      : await snapshotPlan(ctx.cwd, invocation.planPath);
  ...
  if (
    invocation.mode === "autopilot" &&
    compilation.quality !== "executable"
  ) {
  ...
  const graph =
    plan === undefined ? loadedGraph : { ...loadedGraph, entry: "plan-check" };
  ...
  await atomicWrite(
    join(job.directory, "previous-head.txt"),
    `${previousHead ?? ""}\n`,
  );
