...
function accountLines(document: AccountsDocument): string[] {
  return Object.entries(document.pools).flatMap(([poolId, pool]) =>
    pool === undefined
      ...
      : pool.slots.map(
          (slot) =>
            `${poolId}/${slot.id}  ${slot.kind}  ${slot.label ?? slot.id}`,
        ),
  );
}
...
async function showAccounts(
  ...
  context.ui.notify(
    lines.length === 0 ? "No accounts configured" : lines.join("\n"),
    "info",
  );
}
...
function resolveSlotReference(
  ...
  const matches = Object.entries(document.pools).flatMap(([poolId, pool]) =>
    pool?.slots.some((slot) => slot.id === reference)
      ? [{ poolId: poolId as PoolId, slotId: reference }]
      : [],
  );
  if (matches.length !== 1) {
    throw new Error(
...
export function registerAccounts(
  ...
  const resolved: Required<AccountsDependencies> = {
    ...
    login:
      ...
      ((providerId, slotId, context) =>
        loginWithOfficialProvider(providerId, slotId, context, (url) =>
          openAuthUrl(pi, url),
        )),
  };
  ...
  pi.on("before_provider_headers", async (event, context) => {
    ...
    const credential = (await resolved.store.readSecrets())[
      `${active.poolId}/${active.slot.id}`
    ];
    const token =
  ...
  pi.on("after_provider_response", async (event, context) => {
    ...
    if (next !== undefined && next.poolId !== source.provider) {
      const model = balancer.findFallbackModel(
        source,
        next.poolId,
